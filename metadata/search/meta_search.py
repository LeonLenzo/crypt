#!/usr/bin/env python3
"""
metadata/search.py — resolve BioProject identity (title/DOI/PMID/PMCID),
...
Outputs:
  metadata/search/data/bioprojects.json  — title, description,
  ...
  metadata/search/data/biosamples.json   — BioSample XML/ENA attrs

Cache:
  metadata/search/data/bp_cache.json      — per-BP, resumable
  metadata/search/data/bs_cache.json       — per-BioSample, resumable
  metadata/search/data/serper_cache.json   — raw Serper results
...
Run from crypt/:
  python metadata/search.py --migrate-legacy   # once: import old caches
  python metadata/search.py                    # fetch remaining / new BPs
  python metadata/search.py --retry             # re-attempt unresolved BPs
  python metadata/search.py --fetch-ena         # fill in ENA/DDBJ BioSample attrs
  python metadata/search.py --report            # show resolution breakdown
"""
...
def main() -> None:
    ...
    if not RUNS_TSV.exists():
        sys.exit(f"ERROR: {RUNS_TSV} not found — run stat/filter.py first")
