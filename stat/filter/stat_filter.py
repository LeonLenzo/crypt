#!/usr/bin/env python3
"""
stat/filter.py — gate, detect eukaryotic co-pathogens, annotate.
...
Inputs:
  stat/build/data/phibase_db.json
  stat/fetch/data/stat_cache.jsonl   (new object format)
  stat/fetch/data/{mode}_accessions.txt

Output:
  stat/filter/data/runs.tsv

Run from crypt/:
    python stat/filter.py
    python stat/filter.py --mode mal
    python stat/filter.py --skip-validate
"""
import argparse
import csv
import json
...
OUT_DIR    = Path("stat/filter")
DB_PATH    = Path("stat/build/data/phibase_db.json")
JSONL_PATH = Path("stat/fetch/data/stat_cache.jsonl")
ACC_DIR    = Path("stat/fetch/data")
