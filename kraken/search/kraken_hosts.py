...
Usage:
    python kraken/search.py --audit          # report the host list, fetch nothing
    python kraken/search.py                  # select, write host_candidates.tsv
    python kraken/search.py --download       # select + fetch CDS
"""
...
import kraken_search as S  # noqa: E402

OUT_DIR   = Path("kraken/search")
DATA_DIR  = OUT_DIR / "data"
RUN_LIST  = Path("kraken/select/data/run_list.tsv")
...
def main():
    ...
    if args.download:
        ...
        print("Next: python kraken/search.py --download")
