#!/usr/bin/env python3
"""
04_filter_meta.py — join BioSample metadata with study design classification.
...
study design, LLM classification) is joined as repeated columns.
...
Output:
  output/04_filter_meta/data/biosample_meta.tsv

Usage:
  python 04_filter_meta.py
  python 04_filter_meta.py --hc    # restrict to same_genus_secondary=False
"""
...
BIOPROJECTS_PATH = Path("output/03a_fetch_xml/data/bioprojects.json")
BIOSAMPLES_PATH  = Path("output/03a_fetch_xml/data/biosamples.json")
LITERATURE_PATH  = Path("output/03b_fetch_literature/data/literature.json")
LLM_PATH         = Path("output/04_filter_meta/data/llm_classifications.jsonl")
...
OUTPUT_FIELDS = [
    ...
    # BioProject LLM classification
    "llm_treatment", "llm_study_setting", "llm_named_pathogen",
    "llm_stat_present", "llm_stat_dominant", "llm_confidence", "llm_rationale",
...
def main() -> None:
    ...
    out_tsv = OUT_DIR / "data" / "biosample_meta.tsv"
    ...
    # merge BioProject XML + literature into a single cache per BioProject
        ...
        with open(LLM_PATH) as f:
            for line in f:
                ...
                if line:
                    rec = json.loads(line)
                    llm[rec["bp"]] = rec
        print(f"Loaded {len(llm):,} LLM classifications", flush=True)
    else:
        print("No LLM classifications found — llm_* columns will be blank", flush=True)
    ...
    for run in rep_rows:
        ...
        # named_host: BioSample XML host attr → title → abstract → description → methods
        named_host, named_host_source = "", ""
        if host_index:
        ...
        results.append({
            ...
            # LLM
            "llm_treatment":       llm.get(bp, {}).get("treatment", ""),
            "llm_study_setting":   llm.get(bp, {}).get("study_setting", ""),
            "llm_named_pathogen":  llm.get(bp, {}).get("named_pathogen", ""),
            "llm_stat_present":    llm.get(bp, {}).get("stat_present", ""),
            "llm_stat_dominant":   llm.get(bp, {}).get("stat_dominant", ""),
            "llm_confidence":      llm.get(bp, {}).get("confidence", ""),
            "llm_rationale":       llm.get(bp, {}).get("rationale", ""),
            # BioProject literature
            "title":               meta.get("title", ""),
    ...
    n_with_methods = sum(1 for r in results if bp_cache.get(r["BioProject"], {}).get("methods_text"))
