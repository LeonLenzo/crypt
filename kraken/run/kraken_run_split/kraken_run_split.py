...
Output:
    kraken/run/kraken_run_split/data/host_taxid_to_accession.json  (tracked — every
        candidate taxid's downloaded accession; owned here now, not by select)
    kraken/run/kraken_run_split/data/index/{aligner}/{taxid}/     (gitignored —
        one index dir per (aligner, host taxid) pair)
    kraken/run/kraken_run_split/data/reads/{run}_{1,2}.fastq.gz    (gitignored —
    ...
    kraken/run/kraken_run_split/data/split_results.tsv             (tracked — one
...
sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent / "db" / "kraken_db_search"))
from kraken_db_search import download_cds, datasets_query, quality_key, scaffold_plus

# ── paths ──────────────────────────────────────────────────────────────────────

SELECT_DATA     = Path("kraken/run/kraken_run_select/data")
RUN_LIST        = SELECT_DATA / "run_list.tsv"
SELECT_READS_DIR = SELECT_DATA / "reads"
HOST_CDS_DIR    = Path("kraken/db/kraken_db_hosts/data/cds/host")

OUT_DIR        = Path("kraken/run/kraken_run_split")
