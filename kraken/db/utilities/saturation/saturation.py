...
Run from crypt/:
    python kraken/db/utilities/saturation.py \\
...
Outputs:
    kraken/db/utilities/saturation/data/sketches/{accession}.npy   (gitignored — cache)
    kraken/db/utilities/saturation/data/sketch_index.tsv           (tracked — what was sketched)
    kraken/db/utilities/saturation/data/saturation_curve.tsv       (tracked — the curves)
    kraken/db/utilities/saturation/data/saturation_summary.tsv     (tracked — one row per taxon)
"""
...
BUSCO_TSV = Path("kraken/db/utilities/busco/data/busco_scores.tsv")
DEFAULT_GENOMES_DIR = Path("kraken/db/search/data/cds/pathogen")

OUT_DIR      = Path("kraken/db/utilities/saturation")
