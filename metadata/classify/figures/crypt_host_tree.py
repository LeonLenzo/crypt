...
(which used stat/filter's raw, sometimes-generic STAT-inferred `host`
...
(see metadata/classify/figures/sample_funnel_v3.py for that).

Must use system python3, not miniconda Python (ete3/sqlite3 incompatibility).

Run as: python3 metadata/classify/figures/crypt_host_tree.py   (from crypt/)

Inputs:
  metadata/classify/data/samples.tsv

Outputs:
  metadata/classify/figures/crypt_host_tree.nwk
  metadata/classify/figures/crypt_host_tree_meta.tsv
"""
...
SAMPLES_TSV         = Path("metadata/classify/data/samples.tsv")
OUT_NWK             = Path("metadata/classify/figures/crypt_host_tree.nwk")
OUT_META            = Path("metadata/classify/figures/crypt_host_tree_meta.tsv")
