#!/usr/bin/env Rscript
# crypt_host_tree.R — plant host species census, field/aerial validation cohort
# Run from crypt/ : Rscript metadata/meta_classify/figures/crypt_host_tree.R
#
# Rebuilt 2026-09-01 from the retired metadata/legacy/figures/crypt_host_tree.R.
# Two changes from the original: (1) reads the new crypt_host_tree.py's simpler
# n_biosamples column instead of n_single/n_multi, since differentiating
# coinfected vs not isn't the point of this figure anymore — it's a host
# census of the 2,719-sample field/aerial cohort, not a co-infection-rate
# figure (see sample_funnel_v3.py for that); (2) one green bar per tip instead
# of two (single/multi) bars.
...
NWK      <- "metadata/meta_classify/figures/crypt_host_tree.nwk"
META     <- "metadata/meta_classify/figures/crypt_host_tree_meta.tsv"
OUT      <- "metadata/meta_classify/figures/crypt_host_tree"
MIN_RUNS <- 1      # minimum BioSamples to include a tip
BAR_UNIT  <- 7     # x-units per log10 BioSample unit
BAR_LW    <- 4.0   # linewidth for the bar (mm) — wider now there's only one per tip
Y_OFF     <- 0      # single centered bar per tip, no stacking offset needed

COL_HOST <- "#27ae60"   # green — BioSamples resolved to this host

# ── Load + filter ─────────────────────────────────────────────────────────────
tree <- read.tree(NWK)
meta <- read.delim(META, stringsAsFactors = FALSE) %>%
  mutate(n_biosamples = as.integer(n_biosamples),
         family       = ifelse(family == "", NA_character_, family))

keep <- meta %>%
  filter(n_biosamples >= MIN_RUNS) %>%
  pull(label)
tree <- drop.tip(tree, setdiff(tree$tip.label, keep))
n_tips <- Ntip(tree)
cat(sprintf("Tips after filter (>=%d BioSamples): %d\n", MIN_RUNS, n_tips))
...
p <- p + geom_segment(
  data      = tip_dat,
  aes(x = BAR_RING, xend = bar_end, y = y, yend = y),
  color     = COL_HOST, linewidth = BAR_LW, na.rm = TRUE
)
...
# ── Scales + theme ────────────────────────────────────────────────────────────
total_biosamples <- sum(meta$n_biosamples, na.rm = TRUE)
...
p_rot <- p_rot +
  ...
  labs(
    title    = "Plant Host Species — Field Validation Cohort",
    subtitle = sprintf("%d BioSamples across %d host species (log₁₀ scale)",
                        total_biosamples, n_tips)
  )
