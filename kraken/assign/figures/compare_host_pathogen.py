...
Inputs:
  ...
  stat/filter/data/runs.tsv   – STAT euk% per run
...
PHIBASE     = ROOT / "stat/build/data/phibase_db.json"
OUT_DIR     = Path("kraken/assign/figures")
...
RUNS_TSV = ROOT / "stat/filter/data/runs.tsv"
stat_data = {}
...
stat_raw = {}
with open(ROOT / "stat/fetch/data/stat_cache.jsonl") as f:
    for line in f:
        line = line.strip()
        if not line:
            continue
