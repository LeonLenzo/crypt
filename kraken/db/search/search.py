...
Output:
    kraken/db/kraken_db_search/data/ref_candidates.tsv   (tracked)
    kraken/db/kraken_db_search/data/pangenome.tsv        (tracked, --scope only)
    kraken/db/kraken_db_search/data/genus_fill.tsv       (tracked, --scope only)
    kraken/db/kraken_db_search/data/cds/pathogen/{accession}/  (gitignored, --download only)
"""
...
DB_PATH      = Path("stat/stat_build/data/phibase_db.json")
RUNS_TSV     = Path("stat/stat_filter/data/runs.tsv")
OUT_DIR      = Path("kraken/db/kraken_db_search")
...
def main():
    ...
    ap.add_argument("--genomes-dir", default=str(DEFAULT_GENOMES_DIR),
                    help="CDS download directory (default: kraken/db/kraken_db_search/data/cds/pathogen)")
