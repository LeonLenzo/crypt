...
Run on Setonix (requires the bbmap and/or hisat2 module + NCBI datasets CLI):
    ...
    python kraken/utilities/split.py --build-index                    # stages 0+1 only
    python kraken/utilities/split.py --aligner hisat2                 # all three stages, HISAT2
    python kraken/utilities/split.py --limit 5                        # smoke test

Output:
    kraken/utilities/split/data/host_taxid_to_accession.json  (tracked — every
        candidate taxid's downloaded accession; owned here now, not by select)
    kraken/utilities/split/data/index/{aligner}/{taxid}/     (gitignored —
        one index dir per (aligner, host taxid) pair)
    kraken/utilities/split/data/reads/{run}_{1,2}.fastq.gz    (gitignored —
    ...
    kraken/utilities/split/data/split_results.tsv             (tracked — one
...
from kraken_search import download_cds, datasets_query, quality_key, scaffold_plus

# ── paths ──────────────────────────────────────────────────────────────────────

SELECT_DATA     = Path("kraken/select/data")
RUN_LIST        = SELECT_DATA / "run_list.tsv"
SELECT_READS_DIR = SELECT_DATA / "reads"
HOST_CDS_DIR    = Path("kraken/search/data/cds/host")

OUT_DIR        = Path("kraken/utilities/split")
...
def main():
    ...
    try:
        ...
        print(f"Next: python kraken/assign.py")
    finally:
        log.close()
