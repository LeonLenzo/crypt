...
Run from crypt/:
    python kraken/utilities/busco.py \\
        --busco-db-path /scratch/pawsey1168/llenzo/crypt/kraken/utilities/busco/data/busco_downloads \\
        [--genomes-dir /scratch/pawsey1168/llenzo/crypt/kraken/search/data/cds/pathogen] \\
        [--busco-out   /scratch/pawsey1168/llenzo/kraken/busco] \\
        [--workers 16] [--cpus-per-busco 8]
        [--fungi-threshold 50.0] [--oomycete-threshold 65.0]

Outputs:
    kraken/utilities/busco/data/busco_scores.tsv        (tracked — final merged table)
    kraken/utilities/busco/data/busco_scan_cache.tsv    (tracked — resumable raw score cache)
...
CANDIDATES_TSV = Path("kraken/search/data/ref_candidates.tsv")
DEFAULT_GENOMES_DIR = Path("kraken/search/data/cds/pathogen")
DEFAULT_BUSCO_DB    = Path("kraken/utilities/busco/data/busco_downloads")
DEFAULT_BUSCO_OUT   = Path("/scratch/pawsey1168/llenzo/kraken/busco")  # ephemeral, cleared per-run

OUT_DIR    = Path("kraken/utilities/busco")
...
def run_finalize(candidates: list, thresholds: dict) -> None:
    ...
    print(f"\nOutput: {FINAL_TSV}")
    print(f"Next:   python kraken/build.py")
