...
Run from crypt/ (requires kraken2 and datasets CLI in PATH):
    python kraken/db/build.py                              # default dirs
    python kraken/db/build.py --db-dir PATH --genomes-dir PATH   # Setonix scratch
...
Output:
    ...
    kraken/db/build/logs/  build log
"""
...
BUSCO_SCORES = Path("kraken/db/utilities/busco/data/busco_scores.tsv")
OUT_DIR      = Path("kraken/db/build")
...
def main() -> None:
    ...
    ap.add_argument("--host-genomes-dir",
                    default="kraken/db/search/data/cds/host",
