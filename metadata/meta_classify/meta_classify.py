...
Output: metadata/meta_classify/data/samples.tsv
        one row per biosample_representative BioSample, ONLY for full-text BioProjects

Cache: metadata/meta_classify/data/classify_cache.jsonl        (per-BioProject)
       metadata/meta_classify/data/host_disambig_cache.jsonl   (per-BioSample)
...
BIOPROJECTS = Path("metadata/meta_search/data/bioprojects.json")
BIOSAMPLES  = Path("metadata/meta_search/data/biosamples.json")
RUNS_TSV    = Path("stat/stat_filter/data/runs.tsv")
PHIBASE_DB  = Path("stat/stat_build/data/phibase_db.json")
OUT_DIR     = Path("metadata/meta_classify")
