#!/usr/bin/env python3
"""
metadata/classify.py — LLM BioProject classification on FULL manuscript text.
...
Output: metadata/classify/data/samples.tsv
        one row per biosample_representative BioSample, ONLY for full-text BioProjects

Cache: metadata/classify/data/classify_cache.jsonl        (per-BioProject)
       metadata/classify/data/host_disambig_cache.jsonl   (per-BioSample)
       both append-only JSONL

Requires: OPENAI_API_KEY env var

Run from crypt/:
  python metadata/classify.py
  python metadata/classify.py --workers 8
  python metadata/classify.py --rerun-all
  python metadata/classify.py --focus setting     # rerun one dimension only
  python metadata/classify.py --disambiguate-hosts  # per-BioSample host pass
"""
...
BIOPROJECTS = Path("metadata/search/data/bioprojects.json")
BIOSAMPLES  = Path("metadata/search/data/biosamples.json")
RUNS_TSV    = Path("stat/filter/data/runs.tsv")
PHIBASE_DB  = Path("stat/build/data/phibase_db.json")
OUT_DIR     = Path("metadata/classify")
...
def main() -> None:
    ...
    for path, hint in [
        (BIOPROJECTS, "run metadata/search.py first"),
        (BIOSAMPLES,  "run metadata/search.py first"),
        (RUNS_TSV,    "run stat/filter.py first"),
        (PHIBASE_DB,  "run stat/build.py first"),
    ]:
        if not path.exists():
