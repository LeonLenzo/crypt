...
Inputs:
  ...
  stat/stat_filter/data/runs.tsv   – STAT euk% per run
...
PHIBASE     = ROOT / "stat/stat_build/data/phibase_db.json"
OUT_DIR     = Path("kraken/run/kraken_run_assign/figures")
...
RUNS_TSV = ROOT / "stat/stat_filter/data/runs.tsv"
stat_data = {}
...
stat_raw = {}
with open(ROOT / "stat/stat_fetch/data/stat_cache.jsonl") as f:
    for line in f:
        line = line.strip()
        if not line:
            continue
