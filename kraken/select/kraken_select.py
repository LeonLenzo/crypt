...
them itself.
  ...
  1. SELECT — filter metadata/classify/data/samples.tsv down to the
     ...
     stat/filter/data/runs.tsv on BioSample to
...
Run on Setonix (requires sra-tools: prefetch, fasterq-dump):
    python kraken/select.py
    python kraken/select.py --setting field,greenhouse --limit 50   # broader/smaller test

Output:
    kraken/select/data/run_list.tsv   (tracked — Run/BioSample/host/
        candidate_host_taxids/status; kraken_run_split.py consumes this to
        resolve+download host genomes itself)
    kraken/select/data/reads/{run}_1.fastq.gz [+ _2]  (gitignored)
"""
...
def main():
    ...
    try:
        ...
        if not args.download:
            ...
            print("Next: python kraken/utilities/split.py "
