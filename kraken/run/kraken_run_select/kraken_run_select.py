...
them itself.
  ...
  1. SELECT — filter metadata/meta_classify/data/samples.tsv down to the
     ...
     stat/stat_filter/data/runs.tsv on BioSample to
...
Output:
    kraken/run/kraken_run_select/data/run_list.tsv   (tracked — Run/BioSample/host/
        candidate_host_taxids/status; kraken_run_split.py consumes this to
        resolve+download host genomes itself)
    kraken/run/kraken_run_select/data/reads/{run}_1.fastq.gz [+ _2]  (gitignored)
"""
...
OUT_DIR   = Path("kraken/run/kraken_run_select")
