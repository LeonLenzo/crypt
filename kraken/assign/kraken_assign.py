...
runs).
    ...
    python kraken/assign.py \\
        --runs-tsv  kraken/select/data/run_list.tsv \\
        --reads-dir kraken/select/data/reads \\
        --db        kraken/build/data/db_v2

Output: kraken/assign/data/kraken_cache.jsonl
        kraken/assign/data/kraken_cache_index.txt
"""
...
def main() -> None:
    ...
    ap.add_argument("--runs-tsv", default=None, metavar="PATH",
                    help="Read run IDs from stat/filter.py's runs.tsv output.")
